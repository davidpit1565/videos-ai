...
| 35 | "An automation built in n8n can already say 'done' — and still go do the exact same thing again." | Contrarian Open (last used episode 30, not back-to-back) — contradicts the natural assumption that "done" means it only happened once; the mechanism is the Webhook node's default "wait for the whole workflow to finish" setting |
| 36 | "Every private thing you've ever typed into ChatGPT trains its next model — and that's the default." (reworded from a flatter first draft, "ChatGPT trains itself on everything you've ever typed to it," to lead with the stake — "private" — rather than the mechanism, per the @rpn stakes-escalation formula) | You-Focused Appeal (last used episode 33, not back-to-back) — first episode built under the 16.9.2026 reach-first/plain-language standing decision (general audience over jargon-heavy insider topics, even against higher measured YouTube demand) |
