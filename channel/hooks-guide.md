...
| 34 | "That n8n node you built didn't run once. It ran once for every item you sent it." | Shock/Surprise (not used since episode 29), rotated off 33's You-Focused Appeal + Specific Number — genuinely new mechanism (per-item execution model, not error-handling or memory) so it doesn't repeat episodes 7/25/29/32/33's angles on n8n, checkable claim (per-item execution, confirmed against current n8n docs) lands in the first two sentences |
