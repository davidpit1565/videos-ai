...
| 35 | "Says \"done.\" Does it again." (logged retroactively 16.9.2026 — missed at ship time) | Shock/Surprise — same type as 34, back-to-back, which breaks this file's own "never repeat the same hook type two episodes in a row" rule. Not being reshot after the fact; logged plainly as a miss so episode 36 rotates off it correctly instead of compounding the error. |
| 36 | "Before you send that text, paste it here first." (script draft, `channel/episode-36-script.md` — not yet produced) | Direct Address/Question — rotated off 35's Shock/Surprise (last used at episode 31, five episodes off), imperative pointing at something the viewer can do immediately, not a hypothetical — passes the dry-sentence test on a real, doable action rather than an "would you even know" framing. |
